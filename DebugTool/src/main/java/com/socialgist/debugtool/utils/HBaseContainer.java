...
import com.socialgist.debugtool.utils.items.GvpServer;
import com.socialgist.debugtool.utils.items.GvpTokenItem;
...
import com.socialgist.debugtool.utils.items.GvpVideo.PremiumStatus;
...
//	VK
//****************************************************************	
	@Value("${vk.hbase.table.walls:hbase_dev:vk_walls}")
	String walls_table;
	@Value("${vk.hbase.table.messages:hbase_dev:vk_messages}")
	String messages_table;
	@Value("${vk.hbase.table.comments:hbase_dev:vk_comments}")
	String comments_table;
	
	
	@Value("${gvp.utils.hbase.table.debug_stats:hbase_dev:debug_stats}")
...
//	VK
	private Table hTable_VkWalls;
	private Table hTable_VkMessages;
	private Table hTable_VkComments;
...
//			VK
			if (!walls_table.isEmpty()) {
				hTable_VkWalls = connection.getTable(TableName.valueOf(walls_table));
			}
			if (!messages_table.isEmpty()) {
				hTable_VkMessages = connection.getTable(TableName.valueOf(messages_table));
			}
			if (!comments_table.isEmpty()) {
				hTable_VkComments = connection.getTable(TableName.valueOf(comments_table));
			}
...
//				if ((result != null) && !result.isEmpty()) { // MEDIA FOUND IN HBASE
	...
	public ResultScanner getScanner_TableWalls(Scan scan) throws IOException {
		return hTable_VkWalls.getScanner(scan);
	}
...
// 	VK
// ***************************************************************	
	public synchronized VkWall hBase_getWall(String hbase_id) throws IOException  {
		Get get=new Get(Bytes.toBytes(hbase_id));
		Result result = hTable_VkWalls.get(get);
		if (result == null || result.isEmpty()) 
			return null;
		else 
			return new VkWall(result);  					
	}

	public synchronized VkWall hBase_getWallByRef(String hbase_ref_id) throws IOException  {
		Get get=new Get(Bytes.toBytes(hbase_ref_id));
		Result result = hTable_VkWalls.get(get);
		if (result == null || result.isEmpty()) 
			return null;
		else {
			byte[] hbase_wall_id       = result.getValue(Bytes.toBytes("cfStatus"), Bytes.toBytes("hbase_wall_id"));
			String wall_id = (hbase_wall_id!=null)?Bytes.toString(hbase_wall_id):null;		
			return hBase_getWall(wall_id);
		}
	}
	
	public VkWall checkWallForExistence(String wall_id) throws IOException {
		LOGGER.debug("HBase:checkWallForExistence: " + wall_id);
		VkWall vkWall = null; 
		String hbase_wall_id = "wall." + wall_id;
		vkWall = hBase_getWall(hbase_wall_id);
		return vkWall;
	}

	public synchronized VkPost hBase_getPost(String hbase_id) throws IOException  {
		Get get=new Get(Bytes.toBytes(hbase_id));
		Result result = hTable_VkMessages.get(get);
		if (result == null || result.isEmpty()) 
			return null;
		else 
			return new VkPost(result);  					
	}
	
	public VkPost checkPostForExistence(String post_id) throws IOException {
		LOGGER.debug("HBase:checkPostForExistence: " + post_id);
		VkPost vkPost = null; 
		String hbase_post_id = "post." + post_id;
		vkPost = hBase_getPost(hbase_post_id);
		return vkPost;
	}

	public synchronized VkComment hBase_getComment(String hbase_id) throws IOException  {
		Get get=new Get(Bytes.toBytes(hbase_id));
		Result result = hTable_VkComments.get(get);
		if (result == null || result.isEmpty()) 
			return null;
		else 
			return new VkComment(result);  					
	}
	
	public VkComment checkVkCommentForExistence(String hbase_index) throws IOException {
		LOGGER.debug("HBase:checkPostForExistence: " + hbase_index);
		VkComment vkComment = null; 
		vkComment = hBase_getComment(hbase_index);
		return vkComment;
	}
}
